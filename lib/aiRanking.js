...
//      aiRank / previousAiRank / aiRationale / aiRankDate / aiRuleVersion
//      to each brand.
//
// TREND DIRECTION (previousAiRank):
//   Before this run overwrites a brand's aiRank, we snapshot the current
//   aiRank into previousAiRank. We capture every brand's current rank into
//   an in-memory Map at the start, then write both fields in the same
//   update. The ranking cron is the only writer, so nothing can change a
//   rank between the snapshot and the write. The first run has no prior
//   rank, so previousAiRank stays null and the card reads NEW; movement
//   appears on the second run. Additive only, never deletes.
//
...
export async function rankBrands({ dryRun = true } = {}) {
  try {
    ...
    // Snapshot each brand's current aiRank before anything overwrites it.
    // Built from the pool records, which still carry the existing aiRank.
    // A brand with no numeric rank yet maps to null, so it reads NEW.
    const priorRankByName = new Map(
      pool.map((p) => [
        p.name,
        typeof p.aiRank === 'number' ? p.aiRank : null,
      ])
    );
    ...
    for (const v of valid) {
      // The prior rank came from the snapshot taken before any write this
      // run, so writing previousAiRank and the new aiRank together is safe.
      const previousAiRank = priorRankByName.get(v.name) ?? null;
