...
//   When a term comes back below the floor, we now stamp
//   lastNewsUpdate anyway, without writing a velocity and without
//   touching newsHistory. This records that the brand WAS checked
//   and found too thin, so coverage stays auditable: a stamped
//   lastNewsUpdate with empty newsHistory means "checked,
//   insufficient," not "never checked." The value timeline still
//   lives only in newsHistory, so no real data is lost or polluted.
//   lastNewsUpdate therefore means "last time we looked"; the last
//   time we recorded a real value is the final entry in newsHistory.
//
// HISTORY:
...
// full refresh.
//
// On a successful read we write the velocity, stamp lastNewsUpdate, and
// append to newsHistory. On a below-floor read we still stamp
// lastNewsUpdate (see AUDIT NOTE in the header) but write no velocity and
// leave newsHistory untouched, so "checked but too thin" is recorded
...
async function processRecords(records) {
  ...
  for (const record of records) {
    ...
    if (result !== null) {
      ...
      // Below the floor. Record that we checked, without writing a value
      // or polluting the history timeline. A stamped lastNewsUpdate with
      // empty newsHistory now reads as "checked, insufficient."
      record.lastNewsUpdate = new Date();
      await record.save();
