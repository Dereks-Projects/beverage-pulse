...
//   The brand work list is now the gate survivors, not the whole universe.
//   YouTube is the most quota-constrained signal, so scoping it to survivors
//   matters most here. The daily batch rotates over (top-20 beverages +
//   survivors) instead of (top-20 beverages + every brand), so a full lap is
//   weeks, and no quota is spent on dead brands. The gate returns lean
//   objects, which cannot be saved, so we take survivor NAMES from the gate
//   and load them as real documents to write.
//
...
//
//   A full manual run still exceeds the daily free quota even over
//   survivors, so the daily batch remains the supported path. Brands hit
...
//   - A full run of the survivor set is FAR beyond the daily free quota, so
...
// --------------------------------------------------------------------------
// Load the work list: the dormant top-20 beverages plus the gate survivors,
// as savable documents. The gate's records are lean and cannot be saved, so
// we take survivor NAMES and re-load them by name, sorted by rank for a
// stable, even rotation.
//
// Returns { records, survivorCount, gateSummary }.
// --------------------------------------------------------------------------
async function loadWorkList() {
  ...
  const { survivorNames, summary } = await buildGate();

  const brands =
    survivorNames && survivorNames.length
      ? await BrandTrend.find({ name: { $in: survivorNames } }).sort({
          rank: 1,
          weekOf: -1,
        })
      : [];
...
//
// The work list is (top-20 beverages + gate survivors). We do one batch of
...
// survivor-scoped list, far smaller than the full universe.
// --------------------------------------------------------------------------
export async function updateYoutubeTrendsBatch() {
  ...
  const { records: allRecords, survivorCount, beverageCount, gateSummary } =
    await loadWorkList();

  if (allRecords.length === 0) {
    console.warn('YouTube: no records to process (no beverages, no gate survivors).');
    return {
      batch: 0,
      totalBatches: 0,
      termsInBatch: 0,
  ...
  console.log(
    ...
    `over ${allRecords.length} terms (${survivorCount} gate survivors + ` +
...
// survivor-scoped list. Use the daily batch path instead unless you have a
...
export async function updateYoutubeTrends() {
  ...
  console.log('Starting YouTube Social Velocity collection over gate survivors...');
  console.log(
    `  Methodology: views-per-day (${ATTENTION_WEIGHT * 100}%) + ` +
    `publication count (${PUBLICATION_WEIGHT * 100}%), ` +
    `floor: ${MIN_BASELINE_VPD} baseline vpd, min ${MIN_VIDEO_COUNT} videos per window`
  ...
  if (allRecords.length === 0) {
    console.warn('No records to process (no beverages, no gate survivors).');
    return {
      successes: 0,
      failures: 0,
      errors: 0,
  ...
  console.log(
    `  Work list: ${survivorCount} gate survivors + ${beverageCount} beverages ` +
