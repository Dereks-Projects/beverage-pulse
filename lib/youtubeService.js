...
//   - Roughly 202 units per term.
//   - A 10-term daily batch is about 2,020 units, comfortably within
//     the free tier.
//   - A full run of the entire brand set is FAR beyond the daily free
//     quota at the current brand count, so it is not safe to run all at
//     once. The daily batch rotation is the supported path until the
//     gate (run YouTube only on survivors) lands.
//
// AUDIT NOTE (2026-06-04):
//   A term can yield no value for two very different reasons:
//     1. A clean check that came back below the data floors (too few
//        videos, or baseline views-per-day too low). A true "checked,
//        empty" result.
//     2. An API error, most commonly quota exhaustion, where we never
//        got a clean look at all.
//   These must not be treated the same. On a clean below-floor result
//   we stamp lastYoutubeUpdate, so coverage is auditable, matching the
//   News service. On an error we stamp NOTHING and leave the record
//   untouched, so a brand we could not check is never falsely recorded
//   as checked. To make the split possible, fetchVelocityForTerm now
//   THROWS on an API error and returns null ONLY for a clean below-floor
//   result.
// ==========================================================================
...
//
// THROWS on any API error (network, quota exhaustion, bad response).
// The caller relies on this split: null means "checked, too thin,"
// while a thrown error means "could not check." See AUDIT NOTE.
//
// Returns { velocity, ... details } on success.
// --------------------------------------------------------------------------
...
//
// WARNING: a full run far exceeds the daily free quota at the current
// brand count. Use the daily batch path instead unless you have a raised
// quota. Brands hit after the quota wall will be recorded as errors, not
